...
# vector_db.py (Cloud-Native Final Version)
...
class VectorDBManager:
    ...
    def _initialize_with_papers_from_gcs(self):
        ...
        """
        try:
            # Authenticates automatically using the service account from secrets
            ...
            if not pdf_blobs:
                logging.warning(f"No PDF files found in GCS bucket '{self.gcs_bucket_name}'. The vector database will be empty.")
                return
            ...
            for blob in tqdm(pdf_blobs, desc="Processing papers from GCS"):
                try:
                    ...
                    # Download PDF content directly into memory
                    ...
                    if not content.strip():
                        logging.warning(f"No text content could be extracted from '{filename}'. It will be indexed with metadata only.")

                    # Find the corresponding .json metadata file in the bucket
                    ...
                    if meta_blob.exists():
                        ...
                        # Create fallback metadata if no JSON is found
                        metadata = {'title': filename.replace('.pdf', '')}

                    metadata['paper_id'] = filename # This is crucial for linking everything
                    
                    # This single call adds the paper to BOTH ChromaDB and Elasticsearch
                    self.add_paper(
                        paper_id=filename,
                        content=content,
                        metadata=metadata
            ...
            logging.error(f"Failed to connect to or list files from GCS bucket '{self.gcs_bucket_name}': {e}")
            st.error(f"Could not access GCS bucket '{self.gcs_bucket_name}'. Please check the bucket name in your secrets and the service account's permissions. Error: {e}")

    def add_paper(self, paper_id: str, content: str, metadata: Dict[str, Any]):
        """Adds a paper's content and metadata to ChromaDB and Elasticsearch."""
        # Add to ChromaDB (Vector Search)
        # Simple chunking strategy. Can be made more sophisticated if needed.
        chunk_size = 2000
        ...
        # Add to Elasticsearch (Keyword Search & Metadata Filtering)
        self.es_manager.index_paper(paper_id=paper_id, metadata=metadata, content=content)
    ...
    def search_by_keywords(self, keywords: List[str], n_results: int = 10) -> List[Dict[str, Any]]:
        ...
        query = " ".join(keywords)
        try:
            results = self.collection.query(query_texts=[query], n_results=n_results * 2, include=["documents", "metadatas", "distances"])
            processed_results = []
            seen_papers = set()
