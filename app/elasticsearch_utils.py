...
class ElasticsearchManager:
    ...
    def _create_index_if_not_exists(self):
        if not self.client.indices.exists(index=self.index_name):
            # --- FIX 1: ADDED URL/LINK FIELDS TO THE MAPPING ---
            # Added "url", "link", and "doi_url" as "keyword" types.
            # "keyword" is ideal for URLs as they shouldn't be broken into smaller words.
            mapping = {
                "properties": {
    ...
    def index_paper(self, paper_id: str, metadata: Dict[str, Any], content: str = ""):
        self._connect()
        try:
            # --- FIX 2: ADDED URL/LINK FIELDS TO THE DOCUMENT BODY ---
            # Now, we get the link information from the metadata and add it to the document
            # that gets stored in Elasticsearch.
            doc_body = {
                "paper_id": paper_id,
