...
def make_citations_clickable(analysis_text: str, papers: list) -> str:
    """
    Formats and makes inline citations clickable based on specific rules.
    - Wraps every citation number in its own square brackets, e.g., [7][8][13].
    - Ensures citations remain clickable hyperlinks.
    - Excludes non-citation numbers (only formats numbers already in brackets).
    - Supports multi-digit citations like [12].
    - Removes duplicate citations within the same sentence.
    """
    if not papers or not analysis_text:
        return analysis_text

    # 1. Create a mapping of citation numbers (as strings) to paper links.
    # Only include citations that have a valid link.
    citation_links = {
        str(i + 1): get_paper_link(paper.get('metadata', {}))
        for i, paper in enumerate(papers)
    }
    valid_citation_numbers = {
        num for num, link in citation_links.items() if link != "Not available"
    }

    # 2. Define a callback function to process each sentence.
    def process_sentence(match):
        sentence = match.group(0)

        # Find all numbers within square brackets in this sentence.
        found_nums = re.findall(r'\[(\d+)\]', sentence)

        # Create an ordered, unique list of *valid* citation numbers.
        # dict.fromkeys is a concise way to deduplicate while preserving order.
        unique_valid_nums = list(dict.fromkeys(
            num for num in found_nums if num in valid_citation_numbers
        ))

        # Remove all original [number] markers and normalize whitespace.
        cleaned_sentence_with_spaces = re.sub(r'\[\d+\]', '', sentence)
        cleaned_sentence = ' '.join(cleaned_sentence_with_spaces.split()).strip()

        # If there are citations to add back...
        if unique_valid_nums:
            # Create the new, clean citation block, e.g., "[7][8][13]"
            citation_block = "".join(f"[{num}]" for num in unique_valid_nums)

            # Re-insert the citation block before the sentence's final punctuation.
            if cleaned_sentence and cleaned_sentence[-1] in '.!?':
                punctuation = cleaned_sentence[-1]
                core_content = cleaned_sentence[:-1].rstrip()
                return f"{core_content}{citation_block}{punctuation}"
            else:
                return f"{cleaned_sentence} {citation_block}".strip()
        
        # If no valid citations were found, return the sentence with any invalid markers removed.
        return cleaned_sentence

    # 3. Process the entire text by applying the callback to each sentence.
    # The regex `(.+?([.!?]|$))` is a robust way to match sentences, including the last one.
    # Using `re.DOTALL` allows sentences to span newlines.
    processed_text = re.sub(r'(.+?([.!?]|$))', process_sentence, analysis_text, flags=re.DOTALL)

    # 4. Final step: Iterate through the processed text and replace the clean
    #    bracketed numbers with their corresponding clickable markdown links.
    final_text = processed_text
    for num in valid_citation_numbers:
        link = citation_links[num]
        # Use a regex to ensure we are replacing the full [num] and not part of another number.
        pattern = rf'\[({num})\]'
