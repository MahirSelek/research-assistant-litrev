...
# Tek yapilmasi gereken citationlarin bracket [] icine alinmasi lazim ve numaralarla citation ayrimi bu kadar.
...
# 03/09 Yukaridaki sorunlari cozmek icin iki farkli deneme: birincisi:
...
import datetime
from dateutil import parser as date_parser
from collections import defaultdict, OrderedDict # Import OrderedDict for stable deduplication
...
USER_AVATAR = "data:image/svg+xml;base64,"
BOT_AVATAR = "data:image/svg+xml;base64,⁸IDVbM]IDMgNi41Ii8+PHBhdGggZD0iTTE4IDNMMTAuNSAxMC41Ii8+PC9zdmc+"
...
def make_citations_clickable(analysis_text: str, papers: list) -> str:
    """
    Makes inline citations clickable, ensures correct bracketing and deduplication,
    and handles parsing of grouped citations. Excludes non-citation numbers.
    """
    if not papers:
        return analysis_text
    
    citation_links = {}
    valid_citation_nums = set() # Store valid citation numbers as strings for efficient lookup
    for i, paper in enumerate(papers):
        citation_num_str = str(i + 1) # Citation numbers are 1-based indices
        meta = paper.get('metadata', {})
        link = get_paper_link(meta)
        if link != "Not available":
            citation_links[citation_num_str] = link
            valid_citation_nums.add(citation_num_str)

    # --- Step 1: Parse and Normalize Grouped Citations within single brackets ---
    # This function handles cases like "[781113]" and converts them to "[7][8][11][13]"
    # It attempts to match the longest possible valid citation numbers first.
    def parse_grouped_citation_string(match_obj):
        raw_citation_string = match_obj.group(1) # e.g., "781113" or "12" or "1"
        
        parsed_numbers = []
        current_index = 0
        while current_index < len(raw_citation_string):
            found_match = False
            # Try to match multi-digit citations first (longest match)
            # Assuming max citation ID length is 3 digits (e.g., [100]), adjust if more papers.
            for length in range(min(3, len(raw_citation_string) - current_index), 0, -1):
                num_str = raw_citation_string[current_index : current_index + length]
                if num_str in valid_citation_nums:
                    parsed_numbers.append(num_str)
                    current_index += length
                    found_match = True
                    break
            
            if not found_match:
                # If no valid multi-digit or single-digit match was found from valid_citation_nums,
                # take the current digit and move on. It won't be made clickable later.
                num_str = raw_citation_string[current_index]
                # Only add to parsed_numbers if it's a known valid citation.
                # This ensures only *actual* citations are considered for bracketing/linking.
                if num_str in valid_citation_nums:
                    parsed_numbers.append(num_str)
                current_index += 1
        
        # Return the individually bracketed numbers (deduplication happens in the next step for sequences)
        return ''.join([f'[{num}]' for num in parsed_numbers])

    # Apply the parsing to any sequence of digits found inside brackets
    # This will convert "[781113]" to "[7][8][11][13]" (if 7,8,11,13 are valid)
    # And "[12]" to "[12]" (if 12 is a valid citation) or "[1][2]" (if 1 and 2 are valid, but 12 isn't)
    analysis_text = re.sub(r'\[(\d+)\]', parse_grouped_citation_string, analysis_text)

    # --- Step 2: Deduplicate sequences of citations ---
    # This handles cases like "[1][1][2][3][2]" and converts them to "[1][2][3]"
    def deduplicate_citation_sequence(match_obj):
        full_sequence = match_obj.group(0) # e.g., "[1][1][2][3][2]"
        
        # Extract all numbers from the sequence of brackets
        citation_numbers_raw = re.findall(r'\[(\d+)\]', full_sequence)
        
        # Deduplicate while preserving order using OrderedDict
        # We only want to deduplicate numbers that were successfully identified as valid citations
        deduplicated_numbers = list(OrderedDict.fromkeys(
            num for num in citation_numbers_raw if num in valid_citation_nums
        ))
        
        # Reconstruct the unique and bracketed citation sequence
        return ''.join([f'[{num}]' for num in deduplicated_numbers])

    # Apply deduplication to any sequence of one or more bracketed citations
    # Example regex: "(?:\[\d+\])+" matches "[1]", "[1][2]", "[12][3][4]"
    analysis_text = re.sub(r'(?:\[\d+\])+', deduplicate_citation_sequence, analysis_text)

    # --- Step 3: Make valid citations clickable ---
    # Iterate through our map of valid citation numbers to their links
    for citation_num, link in citation_links.items():
        # Only create a clickable link for numbers that were identified as valid citations
        # The pattern ensures we only replace bracketed numbers that exactly match a citation ID
        pattern = rf'\[{re.escape(citation_num)}\]' # Use re.escape for safety in regex
        replacement = f'[{citation_num}]({link})'
        analysis_text = re.sub(pattern, replacement, analysis_text)
