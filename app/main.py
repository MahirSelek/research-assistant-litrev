...
# <<< MODIFICATION START: Improved get_paper_link function >>>
def get_paper_link(metadata: dict) -> str:
    """
    Retrieves the paper link from various possible keys in the metadata.
    Checks multiple common link field names and validates the URL.
    ...
    for field in link_fields:
        ...
        if link and isinstance(link, str):
            ...
            if link.startswith('http'):
                return link
            # Handle raw DOIs that don't have the full URL
...
# <<< MODIFICATION START: New function to get full metadata from GCS >>>
def reload_paper_metadata(papers: list) -> list:
    """
    Reloads metadata from .metadata.json files in GCS to ensure links are present.
    The search results might have partial metadata, so this fetches the full version.
    """
    if not papers:
        return papers
    ...
    try:
        ...
        for paper in papers:
            paper_id = paper.get('paper_id')
            if paper_id and paper_id.endswith('.pdf'):
                # Construct the corresponding .metadata.json filename
                json_filename = paper_id.rsplit('.', 1)[0] + '.metadata.json'
                json_blob = bucket.blob(json_filename)
                ...
                if json_blob.exists():
                    try:
                        ...
                        # Update the paper metadata with the full content from the JSON file
                        updated_metadata = paper.get('metadata', {}).copy()
                        updated_metadata.update(json_metadata)
                        ...
                        # If JSON loading fails, keep the original paper data
                        updated_papers.append(paper)
                else:
                    # If no JSON file is found, keep original paper data
                    updated_papers.append(paper)
            else:
                updated_papers.append(paper)
        ...
        # If any GCS error occurs, return original papers to avoid crashing
        return papers
# <<< MODIFICATION END >>>

# <<< MODIFICATION START: Completely rewritten make_citations_clickable function >>>
def make_citations_clickable(analysis_text: str, papers: list) -> str:
    """
    Makes inline citations clickable and cleans up common formatting issues.
    It handles grouped citations, duplicates, and missing brackets before linking.
    """
    if not papers or not analysis_text:
        return analysis_text
    
    import re
    
    # Create a mapping of citation numbers (as strings) to paper links
    citation_links = {}
    for i, paper in enumerate(papers):
        meta = paper.get('metadata', {})
        link = get_paper_link(meta)
        if link != "Not available":
            citation_links[str(i + 1)] = link

    # --- Pre-processing Phase: Clean up common LLM citation mistakes ---

    # Step 1: Add brackets to un-bracketed numbers at the end of sentences
    # e.g., "This was shown in study 1." -> "This was shown in study [1]."
    analysis_text = re.sub(r'(\s)([1-9]\d*)(\s*[.,;!?]|\s*$)', r'\1[\2]\3', analysis_text)
    
    # Step 2: Split incorrectly grouped citations like [123] into [1][2][3]
    # This is done carefully to avoid splitting valid multi-digit citations like [12].
    # We split if a long number like [123] is found, where 1, 2, and 3 are all valid separate citations.
    def split_grouped_citations(match):
        full_number = match.group(1)
        if len(full_number) > 1 and full_number not in citation_links:
            # If the full number isn't a valid citation, try splitting it
            single_digits = list(full_number)
            # Check if all resulting single digits are valid citations
            if all(digit in citation_links for digit in single_digits):
                return ''.join(f'[{d}]' for d in single_digits)
        # Otherwise, return the original match
        return f'[{full_number}]'
    analysis_text = re.sub(r'\[([1-9]\d*)\]', split_grouped_citations, analysis_text)

    # Step 3: Find all sequences of citations (e.g., [2][3][2]), then deduplicate and sort them.
    def process_citation_sequence(match):
        sequence = match.group(0)
        # Find all numbers within the matched sequence (e.g., '2', '3', '2')
        numbers = re.findall(r'\[(\d+)\]', sequence)
        
        # Deduplicate while preserving order and filter out invalid/zero citations
        seen = set()
        unique_valid_numbers = []
        for num in numbers:
            if num in citation_links and num not in seen:
                seen.add(num)
                unique_valid_numbers.append(num)
        
        # Sort the numbers numerically for consistency
        sorted_numbers = sorted(unique_valid_numbers, key=int)

        # Reconstruct the bracketed sequence
        return ''.join([f'[{num}]' for num in sorted_numbers])

    # This regex finds one or more adjacent bracketed numbers
    analysis_text = re.sub(r'(?:\[\d+\]\s*)+', process_citation_sequence, analysis_text)

    # --- Linking Phase: Replace all valid [num] with markdown links ---
    
    # Iterate through the valid citations (from largest number to smallest to avoid replacing '[1]' inside '[10]')
    for num in sorted(citation_links.keys(), key=int, reverse=True):
        link = citation_links[num]
        # Replace [num] with a clickable markdown link. The regex ensures we don't re-link already linked citations.
        pattern = rf'\[({num})\](?!\()'
        replacement = f'[{num}]({link})'
        analysis_text = re.sub(pattern, replacement, analysis_text)
...
def process_keyword_search(keywords: list, time_filter_type: str | None, selected_year: int | None, selected_month: str | None) -> tuple[str | None, list, int]:
    ...
    with st.spinner("Searching for highly relevant papers and generating a comprehensive, in-depth report..."):
        ...
        # <<< MODIFICATION START: Call reload_paper_metadata here >>>
        # This crucial step ensures we have the full metadata (including links) before generating the context.
        top_papers = reload_paper_metadata(top_papers)
        # <<< MODIFICATION END >>>
        ...
        # <<< MODIFICATION START: Enhanced prompt for better citation formatting >>>
        prompt = f"""{context}
---
**CRITICAL TASK:**
...
1.  At the end of every sentence or key finding you derive from a source, you **MUST** include a citation marker referencing the source's number in brackets. Example: `This new method improves risk prediction [1].`
2.  **ALWAYS** separate multiple citations with individual brackets. **CORRECT: `[2][3][4]`**. **INCORRECT: `[2,3,4]` or `[234]`**.
3.  In the "Key Paper Summaries" section, do NOT add citation numbers to the paper titles. Only add citations at the end of the summary paragraphs themselves.
"""
        # <<< MODIFICATION END >>>
        analysis = post_message_vertexai(prompt)
        
        # Make citations clickable using the new robust function
        if analysis:
            analysis = make_citations_clickable(analysis, top_papers)
...
def main():
    ...
    if st.session_state.active_conversation_id is None:
        ...
        for message in active_conv["messages"]:
            avatar = BOT_AVATAR if message["role"] == "assistant" else USER_AVATAR
            with st.chat_message(message["role"], avatar=avatar):
                st.markdown(message["content"], unsafe_allow_html=True) # Allow HTML for clickable links
                        ...
                        if paper_id:
                            ...
                            if pdf_bytes:
                                st.download_button(
                                    label="Download PDF",
                                    data=pdf_bytes,
                                    file_name=os.path.basename(paper_id),
                                    mime="application/pdf",
                                    key=f"download_{active_id}_{paper_id}"
                                )
    ...
    if st.session_state.active_conversation_id and st.session_state.conversations[st.session_state.active_conversation_id]["messages"][-1]["role"] == "user":
        ...
        with st.spinner("Thinking..."):
            ...
            full_prompt = f"""Continue our conversation. You are the Polo-GGB Research Assistant.
Your task is to answer the user's last message based on the chat history and the full context from the paper sources provided below. Use the same citation style as before, placing bracketed numbers at the end of sentences derived from the context.
...
Assistant Response:"""
            ...
            response_text = post_message_vertexai(full_prompt)
            if response_text:
                # Make citations clickable in follow-up responses as well
                response_text = make_citations_clickable(response_text, active_conv.get("retrieved_papers", []))
                active_conv["messages"].append({"role": "assistant", "content": response_text})
                st.rerun()
