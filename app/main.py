...
# # 01/09 Dorduncu deneme - Bu iyi calisiyor ayni sekilde ustteki de ama similarity searchten dolayi gelen makaleler alakali deiglbazilari o yuzden sadece elastic-search olacak sekilde oynayacagim bu kodla- Alttaki oyle olacak ama olmazsa yine bu koda donebilirim
...
#         # <<< MODIFICATION: Call the updated search function and get the top 15 papers and the total count >>>
...
def initialize_session_state():
    if 'es_manager' not in st.session_state:
        st.session_state.es_manager = get_es_manager(cloud_id=ELASTIC_CLOUD_ID, username=ELASTIC_USER, password=ELASTIC_PASSWORD)
    if 'vector_db' not in st.session_state:
        # Vector DB is no longer used for search, but might be for other features. Keep initialization.
        st.session_state.vector_db = get_vector_db(_es_manager=st.session_state.es_manager)
    if 'conversations' not in st.session_state:
        st.session_state.conversations = {}
    if 'active_conversation_id' not in st.session_state:
...
# It now uses ONLY Elasticsearch for a strict keyword search, completely removing the vector search.
def perform_hybrid_search(keywords: list, time_filter_dict: dict | None = None, n_results: int = 100, score_threshold: float = 0.005, max_final_results: int = 15) -> tuple[list, int]:
    """
    Performs a strict keyword search using only Elasticsearch with an 'AND' operator.
    This ensures that only papers containing ALL keywords are returned.
    The results are ranked by Elasticsearch's internal relevance score.
    Returns a tuple: (list of top papers, total number of papers found).
    """
    # Stage 1: Perform the strict 'AND' search using Elasticsearch.
    # This is now the ONLY source of search results.
    es_results = st.session_state.es_manager.search_papers(
        keywords,
        time_filter=time_filter_dict,
        size=n_results, # Fetch up to n_results to get a good count and ranking.
        operator="AND"
    )

    # Stage 2: Process the results.
    total_papers_found = len(es_results)

    # If no papers are found, return immediately.
    if not total_papers_found:
        return [], 0

    # Format the Elasticsearch results into the standard dictionary format
    # that the rest of the application expects. The results are already
    # sorted by relevance score by Elasticsearch.
    formatted_papers = []
    for hit in es_results:
        ...
        formatted_papers.append(doc_content)
    
    # Stage 3: Return the top N papers for the report, along with the total count.
    # We slice the already sorted list to get the top `max_final_results`.
    final_paper_list = formatted_papers[:max_final_results]

    return final_paper_list, total_papers_found


# This function remains unchanged as it correctly handles the output of perform_hybrid_search.
def process_keyword_search(keywords: list, time_filter_type: str | None, selected_year: int | None, selected_month: str | None) -> tuple[str | None, list, int]:
    if not keywords:
        st.error("Please select at least one keyword.")
        return None, [], 0
...
def main():
    ...
    with st.sidebar:
        ...
        with st.form(key="new_analysis_form"):
            ...
            selected_year, selected_month = None, None
            if time_filter_type == "Year":
                # Note: This logic for getting years/months might be slow if the DB is large.
                # It queries all papers just to get dates.
                import pandas as pd
                all_papers = st.session_state.es_manager.get_all_papers_with_field('publication_date')
                dates = [p['metadata'].get('publication_date') for p in all_papers if p['metadata'].get('publication_date')]
                years = sorted(pd.to_datetime(dates, errors='coerce').dropna().year.unique(), reverse=True)
                selected_year = st.selectbox("Select year", years) if years else st.write("No papers with years found.")
            elif time_filter_type == "Month":
