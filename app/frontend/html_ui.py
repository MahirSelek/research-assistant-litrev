...
class HTMLResearchAssistantUI:
    ...
    def inject_css_and_js(self):
        ...
        function reopenSidebar() {
            ...
            if (sidebar) {
                ...
                // Method 2: Force sidebar to be visible
                sidebar.style.display = 'block';
                sidebar.style.visibility = 'visible';
                
                // Method 3: Remove collapsed classes
                sidebar.classList.remove('collapsed');
                sidebar.classList.add('expanded');
                
                // Method 4: Trigger resize event to force layout update
        ...
        // Check if sidebar is collapsed and add reopen button
        function checkSidebarState() {
            const sidebar = document.querySelector('[data-testid="stSidebar"]');
            const mainContent = document.querySelector('[data-testid="stMain"]');
            
            if (sidebar && mainContent) {
                // Check if sidebar is collapsed (hidden or very narrow)
                const sidebarWidth = sidebar.offsetWidth;
                const isCollapsed = sidebarWidth < 50 || sidebar.style.display === 'none';
                
                if (isCollapsed) {
                    // Remove any existing reopen button
                    const existingButton = document.getElementById('sidebar-reopen-btn');
                    if (existingButton) {
                    ...
                    reopenButton.id = 'sidebar-reopen-btn';
                    ...
                    reopenButton.onmouseover = function() {
                        this.style.transform = 'translateY(-2px)';
                        this.style.boxShadow = '0 6px 20px rgba(102, 126, 234, 0.4)';
                    };
                    
                    reopenButton.onmouseout = function() {
                        this.style.transform = 'translateY(0)';
                        this.style.boxShadow = '0 4px 14px rgba(102, 126, 234, 0.3)';
                    };
                    ...
                    reopenButton.title = 'Click to reopen sidebar';
                    
                    // Add button to page
                    document.body.appendChild(reopenButton);
                } else {
                    // Remove reopen button if sidebar is visible
                    const existingButton = document.getElementById('sidebar-reopen-btn');
                    if (existingButton) {
                        existingButton.remove();
                    }
        ...
        // Check sidebar state every 2 seconds
    ...
    def render_main_interface(self):
        ...
        <div id="sidebar-reopen-container" style="position: fixed; top: 20px; left: 20px; z-index: 1000;">
            <button id="manual-sidebar-reopen" onclick="reopenSidebar()" style="
                background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
                color: white;
                border: none;
                border-radius: 8px;
        ...
        function updateManualReopenButton() {
            const sidebar = document.querySelector('[data-testid="stSidebar"]');
            const manualButton = document.getElementById('manual-sidebar-reopen');
            
            if (sidebar && manualButton) {
                const sidebarWidth = sidebar.offsetWidth;
                const isCollapsed = sidebarWidth < 50 || sidebar.style.display === 'none';
        ...
        // Check sidebar state every second
        setInterval(updateManualReopenButton, 1000);
        
        // Initial check
        setTimeout(updateManualReopenButton, 500);
        </script>
        """, unsafe_allow_html=True)
